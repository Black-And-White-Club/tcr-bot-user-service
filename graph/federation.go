...
func (ec *executionContext) resolveEntity(
	...
	case "User":
		...
		case "findUserByDiscordID":
			id0, err := ec.unmarshalNString2string(ctx, rep["discordID"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findUserByDiscordID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindUserByDiscordID(ctx, id0)
			if err != nil {
...
func entityResolverNameForUser(ctx context.Context, rep EntityRepresentation) (string, error) {
	for {
		...
		return "findUserByDiscordID", nil
	}
